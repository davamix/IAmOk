...
| | |
|---|---|
| Closing out | **Phase 5 is not signed off.** Start from **[phases/phase-5-handover.md](phases/phase-5-handover.md)** — the state, everything still open, and a suggested solution for each. Three items need the owner: copy approval, the cross-role dead end, and the 21:00 reminder. |
| Next phase | **Phase 6 — away mode**, once Phase 5 is signed off. Start from **[phases/phase-5-summary.md](phases/phase-5-summary.md)** and its *Prompt to start the next session*. Phase 5 is built, its exit criterion is met on two devices, and all five reviewers have run with their findings applied; it is **not signed off** — every new user-visible string is owed the owner's approval, two of them changes to already-approved copy, and six review findings were left open on purpose, two of which are owner decisions. |
| Previous phase | **Phase 5 — onboarding and pairing · built, exit criterion MET on two devices.** Two phones paired from a cold install using only a shared code (AVD ↔ POCO F3, 2026-08-26); each landed on the correct main screen; **the AVD finally tapped**, closing the half Phase 4 left owed. [ADR-0011](architecture/decisions/0011-creating-an-invite-is-a-function-too.md) added a **fourth** Function, `createInvite` — §8 always said `invites/{code}` was Function-written and the rules enforce it, while §9 listed only `redeemInvite` and §6 asked the client for a write no client may perform. **The device run found two defects nothing else could**: a third FlutterFire plugin rewriting the emulator host, so every callable from a physical handset went to `10.0.2.2` and hung; and the summary screen being unreachable because the router left onboarding as soon as a question was answered. Both fixed, both with regression tests. **All five reviewers then ran and found fourteen more**, almost none of them from a test failing — including a revoked watcher being told their pairing was live, the onboarding flow being escapable by a second door the same-day fix had left latent, and `redeemInvite` about to deploy accepting ~800 concurrent guesses. 1 161 Dart tests, 75 Functions tests, 75 rules tests, 42 mutations across two harnesses all behaving as expected with passing no-op controls. |
