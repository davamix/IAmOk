...
- The Functions emulator's **first** invocation of a session can fail with *"Failed to load function"*, and that is a cold start rather than a code fault — warm it with one `curl` before driving a device. # 2026-09-01: the first `createInvite` from a phone died after a 24-second gap in the runtime's own module resolution, leaving the app on a bare spinner with no message. It read exactly like Phase 5's *"every callable was sent to an address that does not exist on a handset"*, which is a real defect this project has already had once. `functions/lib/index.js` loaded fine under plain `node`, and the very next call — `curl` from the host — answered in 1.4 s.
