...
| | |
|---|---|
| Current phase | **Phase 7 — UI/UX and the health panel · BRIEFED, not started.** Start from **[phases/phase-7-brief.md](phases/phase-7-brief.md)** — its *Prompt to start the session* is written to be used verbatim. Two of its three deliverables — the watcher list and cold-open state — are **already built**; the health panel does not exist at all, and §13's *clock skew* row has no implementation despite three documents describing one. Owner decisions open with the phase: where the panel lives and how an 80-year-old reaches it, what it says when everything is fine, and whether §12's away transition notifications land here. |
| Previous phase | **Phase 6 — away mode · COMPLETE and SIGNED OFF, 2026-09-02.** Start from **[phases/phase-6-summary.md](phases/phase-6-summary.md)** — read *The gate review*, *The device run* and *The second gate* in [phases/phase-6-summary.md](phases/phase-6-summary.md); [phases/phase-6-brief.md](phases/phase-6-brief.md) is what the phase was asked to do. **The first gate found that the feature did not work.** Three of the five reviewers independently found that away could be set **once per person and then never again** — nothing deletes the document when a period *ends*, `from` was immutable on update, and both call sites passed the stale cached period as `existing`, so every later attempt was refused on both sides with copy blaming the reader's choice of day. Three documents said otherwise, including §12's *"to go longer, set it again"*. Fixed by scoping `from`'s immutability to a **period** rather than a person, in the client and the rules, which is what ADR-0001 decision 6 actually argues; both new rules tests were verified by reverting the clause. **Two more defects that produce silence:** `onAwayChanged` could not change a **closed** watched device (the FCM handler never passed `away:`), and the away read had no timeout while being awaited inside `tap()`. **Eleven claims had stopped being true**, most written in the commits that made them false. **The SECOND gate — the same five reviewers over the close-out, 2026-09-01 — found four more.** A `deploy-notes.md` header claiming the live rules were byte-identical to the repo when they were 2 219 bytes behind and still carried the set-once defect; a test recorder knob **no test ever turned**, so the watcher row could report *"Saved."* for a refused write with all 1 370 tests green; the watcher-side write path reached by **no test at all**, leaving the queued-cache asymmetry enforced only by the absence of a call; and a confirmation dialog whose two actions stacked with **zero gap**, destructive directly under safe, at the font scale it was built for. Nine findings applied, each verified by reverting it; four decisions taken, including **[ADR-0012](architecture/decisions/0012-a-queued-away-write-is-cached-on-the-phone-that-wrote-it.md)**. Built: `AwayRecord` (attribution as a **separate type**, so no policy can decide from a name ADR-0003 says cannot be authenticated), `AwayRepository`, the `self_away` accessors the table waited five versions for, schema **v6**, `onAwayChanged` — the **fourth** Function — the picker, the Tap screen's away state, and the watcher list's away row and action. **All three exit criteria are met in tests and on hardware**, and every Phase 6 device-checklist row was run 2026-09-01 on two API 36 AVDs and then the POCO F3, which confirmed the OEM half — 21/21 reminders armed around an away period with no vendor trimming, and the closed-app nudge waking a killed app in **both** directions on HyperOS. **Doze was measured on a real overnight, 2026-09-02**: `device_idle=full` unbroken from 07:52 to 12:34, and the 12:00 reminder — the first day back after an away period — posted at **12:00:00.876**, 876 ms after the armed second, with 0% battery drain. That closed the last device row. **1 396 Dart tests**, **82 rules tests**, **102 Functions tests** plus a fourth no-emulator run, analyze clean, debug APK builds, and **39 mutations / 39 caught / 0 survived** with 14 passing controls. **[phase-6-handover.md](phases/phase-6-handover.md) was the close-out plan and is now historical.** **Owed:** §12's four away transition notifications only — specified and deliberately not built, and neither the deliverable list nor the exit criteria name them. `firestore.rules` is **changed and not deployed**, deliberately: the emulator is the working surface until release. |
| Phase 5 | **Phase 5 — onboarding and pairing · COMPLETE and SIGNED OFF, 2026-08-26.** Two phones paired from a cold install using only a shared code (AVD ↔ POCO F3); each landed on the correct main screen; **the AVD finally tapped**, closing the half Phase 4 left owed. [ADR-0011](architecture/decisions/0011-creating-an-invite-is-a-function-too.md) added a **fourth** Function, `createInvite`. **The device run found two defects nothing else could** — a third FlutterFire plugin rewriting the emulator host, so every callable from a physical handset hung; and the summary screen being unreachable because the router left onboarding as soon as a question was answered. **All five reviewers then found fourteen more**, almost none from a test failing. **The gate closed the rest**, and running the reviewers over the close-out itself found four more defects in it. [phases/phase-5-summary.md](phases/phase-5-summary.md). |
| Phase 4 | **Firebase backbone · steps 1-7 built, all five reviewers run at the gate, two review rounds applied. Not signed off.** [phases/phase-4-summary.md](phases/phase-4-summary.md) carries the standing list: the first Functions deploy, App Check's console half, the live-radio measurement, and what ADR-0008's option 1 costs — the only open *design* decision, and the owner asked for the number rather than the ADR. Its three device rows: **the AVD tapping is now done** (Phase 5, 2026-08-26), the receiving half is still owed isolated, and **forced** deep Doze remains unreachable on HyperOS — which is why the **overnight** run was done instead, 2026-09-02, and it is now DONE (see `testing/device-matrix.md`, *Doze, on a real overnight*). [phases/phase-4-handover.md](phases/phase-4-handover.md) is the mid-phase snapshot and is deliberately frozen — read it for the four things that went wrong, two of them false greens, not for current state. Phase 3 is complete: [phases/phase-3-summary.md](phases/phase-3-summary.md). |
