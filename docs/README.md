...
| | |
|---|---|
| Next phase | **Phase 6 — away mode.** Start from **[phases/phase-6-brief.md](phases/phase-6-brief.md)** — deliverables, what already exists, the decisions not to re-litigate, and how this phase is expected to go wrong. Away is the first feature whose failure mode is **silence**: a period honoured when it should not be, or one that never ends, produces no notification and no error on either side for up to 31 days. One owner copy decision opens it — `TapCopy.away` names nobody while `screens.md` requires every away surface to name who set it. Then **[phases/phase-5-summary.md](phases/phase-5-summary.md)**. Read *Closing the phase* in that file before writing anything — three of the things settled there constrain Phase 6 directly: the empty-audience 21:00 variant threads `hasAudience` through `AlarmScheduler.apply` (away is the other input to the same call), the chooser put a second action behind the Tap screen's *Add someone* right next to the Away control this phase enables, and `PairingRefusal.serverFault` is the pattern any new refusal copy should follow. |
| Previous phase | **Phase 5 — onboarding and pairing · COMPLETE and SIGNED OFF, 2026-08-26.** Two phones paired from a cold install using only a shared code (AVD ↔ POCO F3); each landed on the correct main screen; **the AVD finally tapped**, closing the half Phase 4 left owed. [ADR-0011](architecture/decisions/0011-creating-an-invite-is-a-function-too.md) added a **fourth** Function, `createInvite`. **The device run found two defects nothing else could** — a third FlutterFire plugin rewriting the emulator host, so every callable from a physical handset hung; and the summary screen being unreachable because the router left onboarding as soon as a question was answered. **All five reviewers then found fourteen more**, almost none from a test failing. **The gate closed the rest**: all three owner decisions taken (copy approved with four amendments; a chooser behind *Add someone*, which closes a **cross-role dead end** nothing could reach; an **empty-audience 21:00 reminder**, so it stops promising a family the device does not have), a fourth refusal `serverFault` because *refused is not unreachable*, the release manifest measured a fourth time, `@types/node` pinned to the deployed runtime, two colour pairs measured, and **the mutation harnesses rebuilt into the repo** — where they found four more test gaps, and refused to score twice rather than guess. 1 202 Dart tests, 83 Functions tests, 75 rules tests, analyze clean, debug and release APKs build. |
