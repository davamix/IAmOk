...
| | |
|---|---|
| Next phase | **Phase 6 — away mode.** Start from **[phases/phase-5-summary.md](phases/phase-5-summary.md)** and its *Prompt to start the next session*. Phase 5 is built, its exit criterion is met on two devices, and all five reviewers have run with their findings applied; it is **not signed off** — every new user-visible string is owed the owner's approval, two of them changes to already-approved copy, and six review findings were left open on purpose, two of which are owner decisions. |
