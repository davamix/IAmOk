...
| | |
|---|---|
| Next phase | **Phase 7 — UI/UX and the health panel.** Not briefed yet. |
| Current phase | **Phase 6 — away mode · BUILT, not signed off, 2026-08-27.** Start from **[phases/phase-6-summary.md](phases/phase-6-summary.md)**; [phases/phase-6-brief.md](phases/phase-6-brief.md) is what the phase was asked to do. All three exit criteria are met **in tests** and none on a device — **nothing in this phase has been on a handset**, and the five reviewers have not run. The owner decision it opened with is taken: the away line names who set it **when it wasn't you**, which is the surface §17's *one watcher silences the whole family* mitigation depends on. Built: `AwayRecord` (attribution as a **separate type**, so no policy can decide from a name ADR-0003 says cannot be authenticated), `AwayRepository`, the `self_away` accessors the table waited five versions for, schema **v6**, `onAwayChanged` — the fifth Function, `onDocumentWritten` because every meaningful away change is an update or a delete — the picker, the Tap screen's away state, and the watcher list's away row and action. **`AwayOutcome` deliberately has no "could not reach the server"**: away queues offline by design, so a write that has not confirmed is either in flight or behind a dead radio and both end with it landing — saying it failed would be a false claim about a write that arrives ninety seconds later. `AwayPeriod` came off the mutation harness's *not mutated* list, where it had sat for five phases: **25 mutations, 25 caught, 0 survived**. 1 322 Dart tests, 102 Functions tests, analyze clean, debug APK builds. **Owed:** the device run, all five reviewers, owner approval of the drafted picker and refusal copy, and §12's four away transition notifications, which are specified and deliberately not built. |
